...
#avrdude -F -p attiny45 -P usb -c stk500V2 -U lfuse:w:0xe2:m -U hfuse:w:0x5f:m -U flash:w:/usr/local/sbin/RPI_ShutDownReboot.hex
#
#echo "hfuse burn reset enabled (no RELAIS output on Pin1 PB5), just for testing, chip can be reused"
#avrdude -F -p attiny45 -P /dev/ttyACM0 -c stk500V2 -U lfuse:w:0xe2:m -U hfuse:w:0xdf:m -U flash:w:/usr/local/sbin/RPI_ShutDownReboot.hex
#
